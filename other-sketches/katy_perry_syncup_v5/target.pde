...
int grade(float windowStart,float currentTime,float windowLength) {
  int evaluation;
  float perfectTime = windowStart + (windowLength / 2);
  float miss = abs(currentTime - perfectTime);
  if (miss > windowLength/2) {evaluation = 0;} else if (miss > windowLength/2.3) {evaluation = 1;} else if (miss > windowLength/2.7) {evaluation = 2;} else if (miss > windowLength/5) {evaluation = 3;}
  else {evaluation = 4;};
  return evaluation;
}


void hide() {visible = false; xCenter=-100; yCenter=-100;} //hide it from draw and change its position temporarily
