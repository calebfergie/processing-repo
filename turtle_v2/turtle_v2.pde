...
int lsize = 90, lsize2 = lsize * lsize; // lens size settings
  int u, v, r2; //lens math vars
  float f;
...
void draw() {
  ...
  for (int vd = - lsize; vd < lsize; vd++) {     // looping 100 pixels around the mouse, we have to make sure we wont 
    for (int ud = - lsize; ud < lsize; ud++) {  // be accessing pixels outside the bounds of our array
      ...
      float resultG=0;
      float resultB=0;
      for (int blurX=u- 1; blurX<=u+ 1; blurX++) {     // visit every pixel in the neighborhood
        for (int blurY=v- 1; blurY<=v+ 1; blurY++) {
